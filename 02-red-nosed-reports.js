...
const d1 = [
  [27, 29, 30, 33, 34, 35, 37, 35], // false
  [51, 53, 54, 55, 57, 60, 63, 63], // false
  [87, 90, 93, 94, 98], // false
  [1, 3, 5, 8, 11], // true
  [10, 7, 6, 5, 4, 1], // true
];

const result = data.reduce((acc, report) => {
  let sequence = [];
  for (let i = 0; i < report.length - 1; i++) {
    ...
    const diff = start < last ? num2 - num1 : num1 - num2;
...
const better = data.reduce((acc, report) => {
  const start = report[0];
  const last = report[report.length - 1];
  const isIncreasing = start < last;
